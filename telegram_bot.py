...
def main():
    ...
    async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            "I monitor RRG (Relative Rotation Graph) values for NSE sectors and stocks.\n\n"
            "Commands:\n"
            "/sub_sectors - Subscribe to sector rotation alerts\n"
            "/unsub_sectors - Unsubscribe from sector alerts\n"
            "/sub_nfo - Subscribe to all NFO stock alerts (211 stocks)\n"
            "/unsub_nfo - Unsubscribe from NFO stock alerts\n"
            "/sub_stocks SYM1,SYM2 - Subscribe to stock alerts\n"
            "/unsub_stocks SYM1,SYM2 - Unsubscribe stocks\n"
            "/unsub_all - Unsubscribe from everything\n"
            "/status - Current sector RRG snapshot\n"
            "/check_sectors - Force a fresh sector RRG check\n"
            "/check_stock SYMBOL - Check RRG for one stock\n"
            "/alert_now - Scan and report sector shifts immediately\n"
            "/check_nfo - Trigger NFO batch scan\n"
            "/help - Show this message\n\n"
            "All NFO stocks are processed together each cycle (may take ~3-4 min). "
            "Alerts run after data refresh. You will be notified of quadrant changes."
        )
